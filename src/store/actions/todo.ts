...
export function removeItem(item_id: string) {
  return async (dispatch: any) => {
    try {
      const removeItem: any = await fetch('http://localhost:3001/todo/' + item_id, { method: 'DELETE' });
...
export function markItemAsDone(item_id: string) {
  return async (dispatch: any) => {
    try {
      ...
      return dispatch({ type: 'MARK_ITEM_DONE_SUCCESS', item_id });
    } catch (error) {
...
export function getTodos() {
  return async (dispatch: any) => {
    try {
      const todoList: any = await fetch('http://localhost:3001/todos', { method: 'GET' });
