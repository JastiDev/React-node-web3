...
function TodoList() {
  ...
  const addItemToList = () => {
    ...
    dispatch(addItem(title) as any);
    setTitle('');
  };

  const removeItemFromList = (item_id: string) => {
    dispatch(removeItem(item_id) as any);
  };

  const toggleItemDone = (item_id: string) => {
    dispatch(markItemAsDone(item_id) as any);
  };
  ...
  return (
    <div className='flex justify-center items-center h-screen'>
      <div className='shadow-lg rounded-lg w-[500px]'>
        <ListHeader addItemToList={addItemToList} title={title} setTitle={setTitle} />
        {todo.error !== '' && (
        ...
        <List list={todo.todo_list} removeItemFromList={removeItemFromList} toggleItemDone={toggleItemDone} />
      </div>
    </div>
  );
}
